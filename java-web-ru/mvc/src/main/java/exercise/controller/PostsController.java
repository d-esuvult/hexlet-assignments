...
public class PostsController {
    ...
    public static void update(Context ctx) {
        var id = ctx.pathParamAsClass("id", Long.class).get();
        var post = PostRepository.find(id)
                .orElseThrow(() -> new NotFoundResponse("Entity not " + id + "not found"));
        try {
            var name = ctx.formParamAsClass("name", String.class)
                    .check(value -> value.length() >= 2, "Название не должно быть короче двух символов")
                    .get();
            ...
            post.setName(name);
            PostRepository.save(post);
            ctx.redirect(NamedRoutes.postPath(id));
        } catch (ValidationException e) {
            var name = ctx.formParam("name");
            var body = ctx.formParam("body");
            var newPost = new Post(name, body);
            var page = new EditPostPage(newPost, e.getErrors());
            ctx.render("posts/edit.jte", model("page", page)).status(HttpStatus.UNPROCESSABLE_CONTENT.getCode());
        }
    }
// END
}
